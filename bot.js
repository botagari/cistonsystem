...
const prefix = "$" // تقدر تغيره
  ...
  var sender = ''
   ...
   if(message.content.startsWith("$buy")){
       let user = client.users.get("567694723679125514"); // ايديك
    ...
    setTimeout(() => {
     ...
     message.channel.sendEmbed(fouroulou).then(msg => {
      ...
      .setTimestamp()
        ...
        if(collected.first().content === 'Great S') {
           let mrx = new Discord.RichEmbed()
          .setColor("#36393e")
          .setAuthor(message.author.tag,message.author.avatarURL)
...
client.channels.get("576138897843552295").sendEmbed(embed)
});
...
client.on('guildMemberAdd', member => {
    ...
    let embed = new Discord.RichEmbed()
        ...
        .addField('اهلا اخي' , `مرحبا بك في سيرفر crintos **${member}**`)
        .addField('ايدي حقك :', "**[" + `${member.id}` + "]**" )
                .addField(' انت العضو رقم',`${member.guild.memberCount}`)
               
                  .addField("اسمك هو:",`<@` + `${member.id}` + `>`, true)
                      
                                     .addField(' اسم السيرفر', `${member.guild.name}`,true)
